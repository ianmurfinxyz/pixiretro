...
{
  log::log(log::LVL_INFO, log::msg_xml_parsing, xmlpath);
  doc->LoadFile(xmlpath.c_str());
  if(doc->Error()){
    log::log(log::LVL_ERROR, log::msg_xml_fail_parse, xmlpath); 
    log::log(log::LVL_INFO, log::msg_xml_tinyxml_error_name, doc->ErrorName());
    log::log(log::LVL_INFO, log::msg_xml_tinyxml_error_desc, doc->ErrorStr());
    return false;
  }
  return true;
}
...
bool extractChildElement(XMLNode* parent, XMLElement** child, const char* childname)
{
  *child = parent->FirstChildElement(childname);
  if(*child == 0){
    log::log(log::LVL_ERROR, log::msg_xml_fail_read_element, childname);
    return false;
  }
  return true;
}
...
bool extractIntAttribute(XMLElement* element, const char* attribute, int* value)
{
  ...
  if(xmlerror != XML_SUCCESS){
    log::log(log::LVL_ERROR, log::msg_xml_fail_read_attribute, attribute);
    return false;
  }
  return true;
}
